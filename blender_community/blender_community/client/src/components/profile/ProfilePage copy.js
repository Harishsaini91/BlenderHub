...
const ProfilePage = () => {
  ...
  return (
    ...
    //   {/* Banner + Basic Info */}
    //   <div className="banner-section" style={{ backgroundImage: `url(http://localhost:5000/uploads/banner/${user.banner})` }}>
    //     <div className="top-edit" onClick={() => handleEdit("basic")}>✏️</div>
    //     <div className="profile-image">
    //       <img src={`http://localhost:5000/uploads/image/${user.image}`} alt="User" />
    //     </div>
    //     <div className="info">
    //       <h2>{user.name}</h2>
    //       {user.bio?.map((line, i) => <p key={i}>{line}</p>)}
    //       <div className="icons">
    //         {user.linkedin?.map((link, i) => (
    //           <a key={i} href={link} target="_blank" rel="noopener noreferrer">🔗 LinkedIn #{i + 1}</a>
    //         ))}
    //         {user.github?.map((link, i) => (
    //           <a key={i} href={link} target="_blank" rel="noopener noreferrer">💻 GitHub #{i + 1}</a>
    //         ))}
    //       </div>
    //     </div>
    //   </div>
